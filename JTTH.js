// Ancestry modifiers lookup (based on X/10 rating system)
// Rating conversion: 1=-25%, 2=-20%, 3=-15%, 4=-10%, 5=0%, 6=+10%, 7-8=+30%, 9=+40%, 10=+50%
...
// Step navigation
on("change:creation_step", function(eventInfo) {
    console.log("creation_step changed from", eventInfo.previousValue, "to", eventInfo.newValue);
});
...
on("clicked:act_step_tab_0", function() {
    console.log("Welcome tab clicked");
    setAttrs({ creation_step: "0" });
});

on("clicked:act_step_tab_1", function() {
    console.log("Ancestry tab clicked");
    setAttrs({ creation_step: "1" });
});

on("clicked:act_step_tab_2", function() {
    console.log("Species tab clicked");
    setAttrs({ creation_step: "2" });
});

on("clicked:act_step_tab_3", function() {
    console.log("Stats tab clicked, setting creation_step to 3");
    setAttrs({ creation_step: "3" });
});

on("clicked:act_step_tab_complete", function() {
    console.log("Complete tab clicked");
    setAttrs({ creation_step: "complete" });
});

// Reset species and save ancestry modifiers when ancestry changes
on("change:ancestry", function(eventInfo) {
    console.log("=== ANCESTRY CHANGE EVENT ===");
    console.log("Previous:", eventInfo.previousValue);
    console.log("New:", eventInfo.newValue);
    console.log("Source:", eventInfo.sourceType);
    
    const ancestry = eventInfo.newValue || "";
    const mods = ancestryModifiers[ancestry] || { cultivation: 0, lifespan: 0 };
